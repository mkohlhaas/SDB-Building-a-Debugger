...
namespace
{
    template <typename T>
    sdb::byte128
    widen(const sdb::register_info &info, T t)
    {
        using namespace sdb;
...
sdb::registers::value
sdb::registers::read(const register_info &info) const
{
...
void
sdb::registers::write(const register_info &info, value val)
{
    auto bytes = as_bytes(data_);
    ...
    std::visit(
        [&](auto &v) {
            ...
            {
                std::cerr << "sdb::register::write called with mismatched register and value sizes";
                std::terminate();
            }
        },
        val);
